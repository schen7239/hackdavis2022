      ...
      <div className="col-span-9 p-10 w-full">
        ...
        <div className="grid grid-cols-3 grid-auto mt-10 w-full gap-x-10 gap-y-10 px-20">
          {mentorsData?.map(({name, description}, index) => (
              <div key={index} className="text-center flex flex-col justify-center place-items-center px-4 py-6 bg-white space-y-2 border-2 rounded-xl shadow-md" >
                <div className="w-[80px] h-[80px] flex items-center justify-center rounded-full bg-red-200">{name[0]}</div>
                <h1 className="text-xl font-semibold">{name}</h1>
                <p className="w-[10rem] text-sm text-gray-400">{description}</p>
                <button className="px-3 py-2 border rounded-lg border-primary-blue text-primary-blue active:scale-90 transition duration-150 ease-out">Connect</button>
              </div>
          ))}
        </div>
      </div>
